...
% $$\delta = j_p N/(2 N_o b)$ modify the shape. 
...
% 
% In general there is a roll-off of high-wavenumber energy, the
% wavenumber of the roll-off moves to lower wavenumbers as the
% energy goes up.  (i.e Gregg et al 93, Polzin 95)
Ef = [0 10 3 1.1 0.3];
clf
for i=1:length(Ef);
  params= Gm76Params;
...
col = {'r','g','b'};
num = 0;
loglog(om,trapz(kz,Ssh),'linewi',1.5);hold on;
for nkz=[5 50 500];
  num = num+1;
  params.Nkz=nkz;
...
legend('Answer','Nkz=5','Nkz=50','Nkz=500')
xlabel('\omega [rad s^{-1}]');
ylabel('\phi_{U}');
