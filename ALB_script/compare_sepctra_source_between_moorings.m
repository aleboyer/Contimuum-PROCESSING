...
i_case=[339 I(normal(ind_kuroshio(41)))];
tt1=[]
for ic=1:length(i_case);
ax(1)=subplot('Position',[.1 .1+(ic-1)*.45 .8 .35]);
    ...
    if ic==1;xlabel(ax(1),'cpd');end;ylabel(ax(1),'m^2s^{-2}/cpd')
...
%                 titlename{ic},MD(i).lat,MD(i).lon,MD(i).depths(ii)))
    ...
    xlabel(ax(2),'yday');ylabel(axyy(1),'Sources (m^2 s^{-2})');ylabel(axyy(2),'E_{GM}/E0_{GM}')
    plot(ax(2),[t1,t1],[0,xmax1],'k--','linewidth',1)
    hold(ax(2),'off')
end  
saveas(gcf,'../figs/source_spectra.png')
