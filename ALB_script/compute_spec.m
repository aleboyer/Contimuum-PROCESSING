...
% very impotant iif is a anomymous function that allows conditional test 
iif = @(varargin) varargin{2 * find([varargin{1:2:end}], 1, 'first')}();
sep_peak = @(x,y) iif(any((x-y) == 0),NaN,...
                      all((x-y)~=0),1);
KEmoor=struct([]);
tic
for m=1:NB_MOOR
%for m=[1 2 3 4]
    ...
    for z=1:Z
        if ~isempty(MD(m).indslab30d{z})
            ...
            KEmoor(m).KE_time{z}=cellfun(@(x,y)...
                cell2mat(cellfun(@(y) mean(x(:,y),2),y,'un',0)),...
                slidingtime,indmslide,'un',0);
            KEmoor(m).freq{z} = A{1}{1};
            KEmoor(m).spec{z} = matmB;
        end
    end
end
%KEmoor.file='compute_spec.m';
toc

save('../alb_mat/MD1_spec.mat','KEmoor','-v7.3')    
