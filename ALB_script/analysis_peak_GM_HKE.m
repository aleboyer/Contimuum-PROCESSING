...
m_grid('box', 'fancy', 'tickdir', 'in', 'FontSize', 12);
hold on
m_scatter(lon(zok>1000),lat(zok>1000),50,meanGM(zok>1000)./GM_E0,'filled');
m_scatter(lon(1566),lat(1566),200,'k','marker','p','linewidth',2);
%caxis([-7 -2]);
colorbar;
caxis([0 10]);
m_coast('patch','k')
title('$\overline{GM}|_{z>1000}$','FontSize', 20, 'Interpreter', 'Latex')
...
m_grid('box', 'fancy', 'tickdir', 'in', 'FontSize', 12);
hold on
m_scatter(lon(zok>1000),lat(zok>1000),50,StdGM(zok>1000)./meanGM(zok>1000)*100,'filled');
m_scatter(lon(1566),lat(1566),200,'k','marker','p','linewidth',2);
%caxis([-7 -2]);
colorbar;
caxis([0 50]);
m_coast('patch','k')
title('$\% \frac{\Delta GM}{\overline{GM}}|_{z>1000}$','FontSize', 20, 'Interpreter', 'Latex')

saveas(gcf,'../figs/map_GM_z1000p.png')
%%
boxlon=[min(lon) max(lon)];boxlat=[min(lat) max(lat)];
figure('Position', [100, 100, 1049, 895]);
colormap(jet)
...
saveas(gcf,'../figs/map_GM_z1000m.png')
%%
boxlon=[min(lon) max(lon)];boxlat=[min(lat) max(lat)];
figure('Position', [100, 100, 1049, 895]);
colormap(jet)
...
m_grid('box', 'fancy', 'tickdir', 'in', 'FontSize', 12);
hold on
m_scatter(lon(ind_ok),lat(ind_ok),20,meanGM(ind_ok)./GM_E0,'filled');
m_scatter(lon(ind_ok(ind_max)),lat(ind_ok(ind_max)),20,'k','marker','p','linewidth',1);
%caxis([-7 -2]);
cax=colorbar;ylabel(cax,'$\frac{\overline{E_{GM}}}{E0_{GM}}$','FontSize', 12, 'Interpreter', 'Latex')
caxis([0 10]);
m_coast('patch','k')
%title('$\overline{GM}$','FontSize', 20, 'Interpreter', 'Latex')
...
m_grid('box', 'fancy', 'tickdir', 'in', 'FontSize', 12);
hold on
m_scatter(lon(ind_ok),lat(ind_ok),20,DeltaGM(ind_ok)./meanGM(ind_ok)*100,'filled');
m_scatter(lon(ind_ok(ind_max)),lat(ind_ok(ind_max)),20,'k','marker','p','linewidth',1);
%caxis([-7 -2]);
cax=colorbar;ylabel(cax,'$\% \frac{\Delta E_{GM}}{\overline{E_{GM}}}$','FontSize', 12, 'Interpreter', 'Latex')
caxis([0 200]);
m_coast('patch','k')
%title('$\% \frac{\Delta GM}{\overline{GM}}$','FontSize', 20, 'Interpreter', 'Latex')

saveas(gcf,'../figs/map_GM_mean_percentDelta.png')
