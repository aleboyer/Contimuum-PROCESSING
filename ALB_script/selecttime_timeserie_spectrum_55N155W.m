...
ax(1)=subplot('Position',[.1 .42 .8 .3]);
...
for s=1:length(selected_loc)
    if(~isempty(selected_loc{s}))
        if (~strcmp(peak(i).name{ii}{iii,s},'f') && ~strcmp(peak(i).name{ii}{iii,s},'M2')...
            ...
            itt=itt+1;
            lM(itt)=plot(ax(2),peak(i).time{ii}{iii},peak(i).KE{ii}{iii,s},...
...
legend([Y1,lM2,lM],{'f','M2',lmname{:}},'location','Northeast','fontsize',8)
...
saveas(gcf,['../figs/exemple_times_spectrum_' num2str(i) '.png'])
