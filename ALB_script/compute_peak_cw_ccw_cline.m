...
%for i=1
    if (~isempty(KEmoor(i).spec)) 
        ...
        TC{it+1}.freq=f;TC{it+1}.name='f';TC{it+2}.freq=2*f;
        ...
        if 1==0
            ...
            print(sprintf('../figs/compute_peak/compute_peaks_%i_%i_%i_cw_ccw_moor_cline.png',i,ii,iii),'-dpng2')
            pause
            close
        end
        
%         % get time
        peak(i).time{ii}{iii}=KEmoor(i).KE_time{ii}{iii};
        ...
        peak(i).f{ii}=f;
        peak(i).freq{ii}{iii}=freq;
        peak(i).Continuum{ii}{iii}=data;        
                    end
                end
            end
        end
...
save(sprintf('../alb_mat/MD1_peak_cw_ccw_cline.mat'),'peak','-v7.3')
